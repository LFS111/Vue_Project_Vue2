    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-if="searchData.categroyname">
              {{ searchData.categroyname }}<i @click="removeCategroyname">×</i>
            </li>
            ...
            <li
              class="with-x"
              v-for="(props, index) in searchData.props"
              :key="index"
            >
              {{ props.split(":")[1] }}<i @click="removeProps(props)">×</i>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    removeCategroyname() {
      this.searchData.categroyname = undefined;
      this.searchData.categroyid1 = undefined;
      this.searchData.categroyid2 = undefined;
      this.searchData.categroyid3 = undefined;
      this.getData();
    ...
    removeKeyword() {
      ...
      this.$bus.$emit("changeKeyword", this.searchData.keyword);
    },
    getTradeInfo(val) {
      this.searchData.trademark = `${val.tmId}:${val.tmName}`;
    ...
    getAttrInfo(attr, attr2) {
      let props = `${attr.attrId}:${attr2}:${attr.attrName}`;
      if (!this.searchData.props.includes(props)) {
    ...
    removeProps(props) {
      this.searchData.props = this.searchData.props.filter((el) => {
        return el != props;
      });
      this.getData();
    },
    changeOrder(flag) {
      // alert(123);
      let orginFlag = this.searchData.order.split(":")[0];
      let orignSort = this.searchData.order.split(":")[1];
      //  let newOrder = "";

      if (orginFlag == flag) {
        this.searchData.order = `${orginFlag}:${
          orignSort == "desc" ? "asc" : "desc"
        }`;
      } else {
        this.searchData.order = `${flag}:"desc"`;
      }

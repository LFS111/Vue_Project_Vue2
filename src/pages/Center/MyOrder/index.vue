        ...
        >
          ...
          <tbody>
            <tr v-for="(cart, index) in order.orderDetailList" :key="cart.id">
              ...
              >
                <ul class="unstyled">
                  <li>总金额¥{{ cart.orderPrice * cart.skuNum }}.00</li>
                  <li>在线支付</li>
                </ul>
              </td>
              <td
...
export default {
  ...
  computed: {
    totalprice() {
      ...
      this.orderLists.forEach((item) => {
        item.orderDetailList.forEach((el) => {
          sum = sum + el.orderPrice * el.skuNum;
        });
      });
      return sum;
    },
  ...
  methods: {
    async getData() {
      ...
      let result = await this.$API.reqOrder(page, limit);
      console.log(result);
      if (result.code == 200) {
        this.orderList = result.data;
        this.orderLists = result.data.records;
      }
    },
    getPageNo(page) {
      this.page = page;
